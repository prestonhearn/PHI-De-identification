...
class DOBRecognizer(PatternRecognizer):
    def __init__(self):
        patterns = [Pattern("DOB", r"(?i)(date of birth|dob|birth date)\s?:?\s?(\d{1,2}\/\d{1,2}\/\d{4}|\d{4}\/\d{1,2}\/\d{1,2}|\d{1,2}-\d{1,2}-\d{4}|\d{1,2}\.\d{1,2}\.\d{4}|\d{4}\.\d{1,2}\.\d{1,2}|\d{4}-\d{1,2}-\d{1,2}|\d{8}|\d{1,2}\s(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s\d{4}|(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s\d{1,2},\s\d{4}|\d{1,2}\s(January|February|March|April|May|June|July|August|September|October|November|December)\s\d{4}|(January|February|March|April|May|June|July|August|September|October|November|December)\s\d{1,2},\s\d{4})", score=1.0)]
